...
def plot_boundaries(X_train, y_train, score=None, probability_func=None, degree = None, n_colors = 100, mesh_res = 200, ax = None, bound_line=True):
    ...
    if probability_func is not None:
        if degree is not None:
            ...
            Z = probability_func(polynomial_set)
            if len(Z.shape)>1:
                Z = Z[:, 1]
        else:
            Z_aux = probability_func(np.c_[xx.ravel(), yy.ravel()])
            if len(Z_aux.shape)>1:
                Z = Z_aux[:, 1]
            else:
                Z = Z_aux
...
def fit_and_get_regions(X_train, y_train, X_test, y_test, degree = 2, lambd = 0, plot_it = True, print_it = False, mesh_res=200, return_model=False):
    ...
    if lambd == 0:
        ...
        C1 = 1/lambd 
    #C2 = 1
    clf_logist_pol = LogisticRegression(C=C1, fit_intercept=False, solver='lbfgs', max_iter=2000)
