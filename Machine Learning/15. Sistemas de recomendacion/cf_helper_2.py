...
def getMeassures(R, R_rated_indexes, R_estimated, accuracy_thres = 0, like = 1, dislike = -1):
...
def getStats(R_train, R_rated_indexes_train, R_test, R_rated_indexes_test, R_estimated, log = True, accuracy_thres = 0, like = 1, dislike = -1):
    acurracy_tr, rmse_tr = getMeassures(R_train, R_rated_indexes_train, R_estimated, accuracy_thres, like, dislike)
    acurracy_te, rmse_te = getMeassures(R_test, R_rated_indexes_test, R_estimated, accuracy_thres, like, dislike)
...
def getUsersMeans_unrated_not_count(R):
    mean_all = R[R.nonzero()].mean()
    means = mean_all*np.ones((R.shape[0],1))
    for user in range(R.shape[0]):
        nonzeros_u = R[user, :].nonzero()
        #print(nonzeros_u[0].shape[0])
        if (nonzeros_u[0].shape[0])>0:
            means[user] = R[user, nonzeros_u].mean()
    return means
...
def calcultaSimilarityMatrix(R, dist_type = 'cosine', max_co_rated = 0,save = False):
    ...
    if dist_type == 'jaccard_with_negatives':
        ...
        similarityMatrix, means = getPearsonSimilarityMatrix(R)
    else:
...
def plot_percentiles(options_vector, rmse_array, perc_array, parameter_name='parameter_name',title= 'Titulo', featured_idx = -1, featured_text = ''):
